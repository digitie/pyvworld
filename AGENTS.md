...
- 불필요한 얇은 wrapper를 새로 만들지 않습니다. 공통화는 실제 중복을 줄이거나 공개 API 계약을 단순하게 만들 때만 추가합니다.
- 다른 라이브러리에 이미 검증된 구현이 있으면 작은 수정에 집착하지 말고 그 구현 방식을 현재 구조에 바로 반영합니다. 라이선스와 출처를 확인하고, 필요한 차이는 테스트와 문서에 남깁니다.
