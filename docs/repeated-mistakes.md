...
## 외부 주소 DTO를 입력 계약으로 만들지 말 것

`python-kraddr-base` 같은 외부 라이브러리의 `PlaceCoordinate`, `Address`는 이 패키지의 런타임 의존성이나 공개 입력이 아닙니다. 호출자는 VWorld가 요구하는 문자열 주소, `x,y` point 문자열, `(lon, lat)` 튜플, 또는 이 패키지의 `LatLon`/`LonLat` 값 객체로 변환해서 전달합니다.

좌표 편의 함수도 외부 DTO나 추가 값 객체를 중간에 만들지 말고 최종 VWorld `x,y` 순서에 맞는 기본 값으로 요청 파라미터를 조립합니다.
